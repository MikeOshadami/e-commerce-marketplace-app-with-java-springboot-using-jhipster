...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query(
        value = "select distinct product from Product product left join fetch product.productCategory left join fetch product.store",
        countQuery = "select count(distinct product) from Product product"
    )
    Page<Product> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct product from Product product left join fetch product.productCategory left join fetch product.store")
    List<Product> findAllWithToOneRelationships();

    @Query(
        "select product from Product product left join fetch product.productCategory left join fetch product.store where product.id =:id"
    )
